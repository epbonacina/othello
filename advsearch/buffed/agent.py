...
prune_over = 7
w1 = 0.7734192456744429
w2 = 1.061766384242463
w3 = 0.38077631629115605
w4 = 1
...
def eval_execute(state: GameState) -> float:
    ...
    total_w = w1+w2+w3
    normal = w1 * actual_mobility(board) + w2 * potential_mobility(board) + w3 * corners_indirect(board) + corners_direct(board)
    return normal

def neg_exe(state):
    return -eval_execute(state)
...
def corners_indirect(board: Board) -> float:
    tiles = board.tiles
    max_corners = 0
    min_corners = 0
...
def corners_direct(board: Board) -> float:
    tiles = board.tiles
    max_corners = 0
    min_corners = 0
    corner_coor = [(0,0), (0,7), (7,0), (7,7)]
